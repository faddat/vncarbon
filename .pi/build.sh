...
# docker buildx build --tag vncarbon --file .pi/Dockerfile --platform linux/arm64 --cache-from vncarbon:cache --cache-to vncarbon:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar vncarbon
...
sudo bash -c "echo vncarbon > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/vncarbon.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/vncarbon.img
